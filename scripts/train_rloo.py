"""RLOO (REINFORCE Leave-One-Out) training script for PlasmidLM post-training.

RLOO uses leave-one-out baselines instead of GRPO's group-mean/std normalization.
This gives honest (un-amplified) advantage signal even with low intra-group diversity.

Usage:
    python scripts/train_rloo.py configs/rloo_g6big.py
"""
...
def build_prompt_dataset(parquet_path: str, filter_hard_tokens: bool = True) -> datasets.Dataset:
    """Build a HF Dataset with a 'prompt' string column for RLOOTrainer."""
...
class RLOOMetricsCallback(TrainerCallback):
    """Print key RLOO metrics to stdout on each log step."""
...
class LineageCallback(TrainerCallback):
    """Log config and data lineage to MLflow."""
    ...
    def on_train_begin(self, args, state, control, model=None, **kwargs):
        ...
        try:
            ...
            params["architecture"] = "plasmid_lm_rloo"
            if model:
...
def main():
    parser = argparse.ArgumentParser(description="Train PlasmidLM with RLOO")
    ...
    # Reward function — same signature as GRPO
    ...
    # RLOO configuration — uses same PostTrainingConfig fields as GRPO
    rloo_config = RLOOConfig(
        ...
        # RLOO-specific (shares some params with GRPO)
    ...
    # Create RLOO trainer
    log.info("Initializing RLOO trainer...")
    callbacks = [RLOOMetricsCallback()]
    if report_to == "mlflow":
        callbacks.append(LineageCallback(config))
    ...
    log.info("Starting RLOO training...")
    trainer.train()
    ...
    log.info("RLOO training complete!")
