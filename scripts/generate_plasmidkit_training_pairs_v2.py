#!/usr/bin/env python3
"""Generate training pairs from PlasmidKit annotations with metadata-derived tokens."""
...
TOKEN_CATEGORIES = [
    ...
    },
    {
        ...
        "field": "growth_strain",
        "tokens": {
            "<SPECIES_BACTERIAL>": ["DH5alpha", "DH10B", "Top10", "Stbl3", "NEB Stable", "ccdB Survival", 
                                   "XL1 Blue", "Mach1", "DB3.1", "JM109", "Pir1", "XL10 Gold", 
                                   "Stbl2", "NEB Turbo", "MG1655", "BL21", "EPI300", "Rosetta",
                                   "Origami", "Shuffle", "T7 Express", "HMS174", "NovaBlue"],
            "<SPECIES_YEAST>": ["BY4741", "BJ5459", "W303", "YPH500", "AH109", "Y2HGold"],
        },
        "other_token": "<SPECIES_OTHER>",
    },
    {
        "category": "ori",
    ...
    },
    {
        "category": "terminator",
        "source": "plasmidkit_annotations",
        "filter_type": "terminator",
        "tokens": {
            "<TERM_T7>": ["T7_terminator"],
            "<TERM_SV40>": ["SV40_terminator"],
            "<TERM_RB>": ["rho_independent_terminator"],
        },
        "other_token": "<TERM_OTHER>",
    },
    {
        "category": "reporter",
        "source": "plasmidkit_annotations",
...
def match_category(pid, cat_def, meta_idx, annot_by_plasmid):
    ...
    multi_match = cat_def.get("multi_match", False)
    matched = []
        ...
        for tok, match_vals in token_defs.items():
            for mv in match_vals:
                if mv in str(raw):
                    ...
                    if not multi_match:
                        break
            if matched and not multi_match:
                break
                
    elif source == "metadata_derived":
        field = cat_def["field"]
        if pid not in meta_idx.index:
            return [other_token] if other_token else []
        raw = meta_idx.at[pid, field]
        if pd.isna(raw):
            return [other_token] if other_token else []
        raw_str = str(raw)
...
def main():
    ...
    print(f"\nSaved {len(df)} training pairs")
    print(f"Avg tokens: {df['num_tokens'].mean():.1f}")
    print(f"Sample: {df.iloc[0]['token_prompt'][:100]}...")
    
    print(f"\n=== Token Usage Statistics ===")
    print(f"Total unique tokens: {len(token_counts)}")
    for tok, count in sorted(token_counts.items(), key=lambda x: -x[1]):
        print(f"  {tok:25s}: {count:6d} ({100*count/len(df):5.1f}%)")
